...
What it does
------------
- Keeps the exact same sheets as the input workbook, in the same order --
  nothing added, nothing removed. Sheets with no role column (Status List,
  Recruiter Performance) are cleaned but never row-filtered.
...
-----
    python extract_sd_analyst_german.py
        Cleans INPUT_PATH below (edit that line first) and writes next to it.

    python extract_sd_analyst_german.py INPUT.xlsx [OUTPUT.xlsx]
        Overrides INPUT_PATH (and OUTPUT_PATH) for this one run, without
        touching the file -- handy for testing a different export.

Output path defaults to "<INPUT>_sd_analyst_german.xlsx" next to the input.
...
# Leave as None to auto-name the output "<INPUT_PATH>_sd_analyst_german.xlsx"
# right next to the input file. Set a full path here instead if you want the
# clean file to always land somewhere specific (e.g. the folder Power BI reads from).
OUTPUT_PATH = None
...
def main():
    # Command-line arguments (if given) override INPUT_PATH / OUTPUT_PATH
    # above -- so `python extract_sd_analyst_german.py` alone uses the path
    # you've edited into the script, no arguments required.
    if len(sys.argv) >= 2:
        input_path = Path(sys.argv[1])
    else:
        input_path = Path(INPUT_PATH)

    if "PASTE_THE_FULL_PATH" in str(input_path):
        ...
        print("\n    python extract_sd_analyst_german.py path\\to\\file.xlsx\n")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        output_path = input_path.with_name(f"{input_path.stem}_sd_analyst_german.xlsx")
